...
do
    ...
    basei_noSuffix=$(echo $basei | awk -F "." '/.sh$/ || /.py$/ {print $1}')
    for filej in $files
    do   
	    ...
	    if [[ -z $basei_noSuffix ]];
	    then
		out=$( grep ${opt} -w -e "${basei}" -e "${basei} " -e "${basei}$" $filej 2>/dev/null )
	    else
		out=$( grep ${opt} -w -e "${basei}" -e "${basei} " -e "${basei}$" -e "${basei_noSuffix}"  -e "${basei_noSuffix} " -e "${basei_noSuffix}$" $filej 2>/dev/null )
	    fi
            if [[ ! -z $out ]];
            then
                if [[ $opt == "-n" ]];
