#!/usr/bin/env python3
'''
  To upload the data and policy files to OPA server.
  It needs:
...
def open_connection() -> OpaClient:
    client = OpaClient(retries=10,timeout=10) # default host='localhost', port=8181, version='v1'
...
def main():
    ...
    if args.data:
        ...
        if cl.update_or_create_opa_data(data, args.pathstr):
            print('{} uploaded'.format(args.data.name))
        else:
            print('{} upload failed'.format(args.data.name))

    if args.policy:
        #cl.delete_opa_policy(args.pathstr)
        if  cl.update_opa_policy_fromfile(args.policy,endpoint=args.pathstr):
            print('{} uploaded'.format(args.policy))
        else:
            print('{} upload failed'.format(args.policy))
