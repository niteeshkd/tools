...
get_digests(event, pcrx, algid) = digests {
    digests := [ dg | dg = tpm2evlog.events[i].Digests[j].Digest ; 
                tpm2evlog.events[i].EventType == event ; 
                tpm2evlog.events[i].PCRIndex == pcrx ;
                tpm2evlog.events[i].Digests[j].AlgorithmId == algid ]
}
