...
KEYLIME_CONTAINER_REGISTRY=`cat /root/cluster.yml | awk -F ":" '/CONTAINER_REGISTRY/ {print $2":"$3}' | tr -d ' '`
KEYLIME_CONTAINER_TAG=`cat /root/cluster.yml | awk -F ":" '/CONTAINER_TAG/ {print $2}' | tr -d ' '`
image="${KEYLIME_CONTAINER_REGISTRY}/ng-keylime:${KEYLIME_CONTAINER_TAG}"
